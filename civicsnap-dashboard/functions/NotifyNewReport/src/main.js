...
export default async ({ req, res, log, error }) => {
    ...
    if (!reportId || !orgId) return res.json({ success: false });
    ...
    try {
        await databases.createDocument(
            ...
            {
                report_id: reportId,
                message: "Nieuwe klacht/melding binnengekomen!",
                org_id: orgId,
                is_read: false
            },
            [Permission.read(Role.team(orgId)), Permission.update(Role.team(orgId))]
        );
